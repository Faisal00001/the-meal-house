    ...
    return (
        <div>
            ...
            p-5 space-y-4 md:space-y-0 items-center'>
                ...
                {/* Category */}
                <section>
                    <h3 className='text-2xl md:text-4xl font-bold text-center mt-5'>Category Types</h3>
                    <div className='mt-7 flex justify-center flex-wrap px-10 gap-5 text-white'>
                        {
                            categories.map(categories => <Link to={`/${categories.strCategory.toLowerCase()}`} key={categories.idCategory}><button className='btn text-white hover:text-black px-5 py-2 bg-rose-500 rounded-lg' >{categories.strCategory}</button></Link>)
                        }
                    </div>
                    <Outlet></Outlet>
